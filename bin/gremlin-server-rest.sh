...
GREMLIN_CONF=$GREMLIN_BIN/gremlin-server-rest.conf
...
if [[ -z "$LOG_FILE" ]]; then
  LOG_FILE="$LOG_DIR/gremlin-rest.log"
fi
...
if [[ -z "$PID_FILE" ]]; then
  PID_FILE=$PID_DIR/gremlin-rest.pid
fi

if [[ -z "$GREMLIN_YAML" ]]; then
  GREMLIN_YAML=$GREMLIN_HOME/conf/gremlin-server/gremlin-server-rest.yaml
fi
