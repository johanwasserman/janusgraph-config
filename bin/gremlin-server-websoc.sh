...
GREMLIN_CONF=$GREMLIN_BIN/gremlin-server-websoc.conf
...
if [[ -z "$LOG_FILE" ]]; then
  LOG_FILE="$LOG_DIR/gremlin-websoc.log"
fi
...
if [[ -z "$PID_FILE" ]]; then
  PID_FILE=$PID_DIR/gremlin-websoc.pid
fi

if [[ -z "$GREMLIN_YAML" ]]; then
  GREMLIN_YAML=$GREMLIN_HOME/conf/gremlin-server/gremlin-server-websoc.yaml
fi
